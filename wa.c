...
Module *load_module(uint8_t *bytes, size_t code_len, Options options) {
  ...
  while (pos < code_len) {
      ...
      for (uint32_t gidx = 0; gidx < import_count; gidx++) {
        ...
        case 0x00:             // Function
          ...
          trace("  import: %s.%s, fidx: 0x%x, type_index: 0x%x\n",
                func->import_module, func->import_field, fidx, type_index);
        ...
        case 0x02:             // Memory
          ...
          trace("  setting memory.bytes to: %p\n", (void*)(*(uint8_t **) val));
    ...
    case 3:
      ...
      functions = arecalloc(m->functions, m->import_count, m->function_count, sizeof(Block), "Block(function)");
      /*if (m->import_count != 0) {*/
    ...
    case 9:
      ...
      for (uint32_t c = 0; c < element_count; c++) {
        ...
        for (uint32_t n = 0; n < num_elem; n++) {
          trace("  write table entries %p, offset: 0x%x, n: 0x%x, addr: %p\n",
    ...
    case 11:
      ...
      for (uint32_t s = 0; s < seg_count; s++) {
        ...
        trace("  setting 0x%x bytes of memory at offset 0x%x\n", size, offset);
