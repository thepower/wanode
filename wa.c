...
void dump_stacks(Module *m) {
    ...
    for (int i = 0; i <= m->csp; i++) {
        ...
        (void) f;trace("%s(sp:%d/fp:%d/ra:0x%x)", block_repr(f->block), f->sp, f->fp, f->ra);
        if (i != m->csp) { trace(" ");
        }
    }trace("]\n");
}
...
void find_blocks(Module *m) {
    ...
    for (uint32_t f = m->import_count; f < m->function_count; f++) {
        function = &m->functions[f];trace("    fidx: 0x%x, start: 0x%x, end: 0x%x\n",
    ...
    while (m->pc < m->byte_count) {
        ...
        switch (opcode) {
            ...
            case 0x04:                 // if
                ...
                // if true, keep going
                if (TRACE) { trace("      - cond: 0x%x jump to 0x%x, block: %s\n",
            ...
            case 0x05:                 // else
                ...
                m->pc = block->br_addr;
                if (TRACE) { trace("      - of %s jump to 0x%x\n", block_repr(block), m->pc);
                }
                continue;
            case 0x0b:                 // end
                block = pop_block(m);
                ...
                if (block->block_type == 0x00) {  // Function
                    ...
                    if (m->csp == ocsp) { trace("Finish interpreting\n");
                        // Return to top-level
                        return true;
                    } else { trace("Keep goint interpreting: csp %d ocsp %d\n", m->csp, ocsp);
            ...
            case 0x0e:                 // br_table
                ...
                m->csp -= depth;
                // set to end for pop_block
                m->pc = m->callstack[m->csp].block->br_addr;trace("      - count: %d, didx: %d, to: 0x%x\n", count,
                                                                  didx, m->pc);
                continue;
            case 0x0f:                 // return
                ...
                m->pc = m->callstack[0].block->end_addr;trace("      - to: 0x%x\n", m->pc);
                continue;
            ...
            case 0x11:                 // call_indirect
                ...
                fidx = m->table.entries[val];trace("       - call_indirect tidx: %d, val: 0x%x, fidx: 0x%x\n",
                                                   tidx, val, fidx);
            ...
            case 0x24:                 // set_global
                arg = read_LEB(bytes, &m->pc, 32);
                m->globals[arg] = stack[m->sp--];trace("      - arg: 0x%x, to %s\n", arg, value_repr(&m->globals[arg]));
                continue;
            ...
            case 0x40:                 // grow_memory
                ...
                m->memory.bytes = arecalloc(m->memory.bytes,
                                            ...
                                            sizeof(uint32_t), "Module->memory.bytes");debug(
            ...
            case 0x35:
                ...
                if (flags != 2 && TRACE) { warn("      - unaligned load - flags: 0x%x,"
                ...
                if (overflow) { warn("memory start: %p, memory end: %p, maddr: %p\n",
            ...
            case 0x3e:
                ...
                addr = stack[m->sp--].value.uint32;
                if (flags != 2 && TRACE) { warn("      - unaligned store - flags: 0x%x,"
...
Module *load_module(uint8_t *bytes, size_t code_len, Options options) {
    ...
    while (pos < code_len) {
        ...
        switch (id) {
            case 0:trace("Parsing Custom(0) section (length: 0x%x)\n", slen);
                ...
                free(name);
                break;
            case 1:trace("Parsing Type(1) section (length: 0x%x)\n", slen);
                ...
                break;
            case 2:trace("Parsing Import(2) section (length: 0x%x)\n", slen);
                ...
                for (uint32_t gidx = 0; gidx < import_count; gidx++) {
                    ...
                    switch (external_kind) {
                        case 0x00:             // Function
                            ...
                            func->type = &m->types[type_index];trace("  import: %s.%s, fidx: 0x%x, type_index: 0x%x\n",
                        ...
                        ASSERT(!m->table.entries, "More than 1 table not supported\n");
                            m->table.entries = val;trace("  setting table.entries to: %p\n",
                                                         (void *) (*(uint32_t **) val));
                ...
                break;
            case 3:trace("Parsing Function(3) section (length: 0x%x)\n", slen);
                m->function_count += read_LEB(bytes, &pos, 32);trace("  import_count: %d, new count: %d\n",
                ...
                for (uint32_t f = m->import_count; f < m->function_count; f++) {
                    ...
                    m->functions[f].type = &m->types[tidx];trace("  function fidx: 0x%x, tidx: 0x%x\n", f, tidx);
                }
                break;
            case 4:trace("Parsing Table(4) section\n");
                ...
                // }
                break;
            case 5:trace("Parsing Memory(5) section\n");
                ...
                // }
                break;
            case 6:trace("Parsing Global(6) section\n");
                ...
                pos = start_pos + slen;
                break;
            case 7:trace("Parsing Export(7) section (length: 0x%x)\n", slen);
                ...
                for (uint32_t e = 0; e < export_count; e++) {
                    ...
                    if (kind != 0x00) { trace("  ignoring non-function export '%s'"
                                              " kind 0x%x index 0x%x\n", name, kind, index);
                        free(name);
                        continue;
                    }
                    m->functions[index].export_name = name;trace("  export: %s (0x%x)\n", name, index);
                }
                break;
            case 8:trace("Parsing Start(8) section (length: 0x%x)\n", slen);
                m->start_function = read_LEB(bytes, &pos, 32);
                break;
            case 9:trace("Parsing Element(9) section (length: 0x%x)\n", slen);
                ...
                for (uint32_t c = 0; c < element_count; c++) {
                    ...
                    for (uint32_t n = 0; n < num_elem; n++) { trace(
            ...
            case 11:trace("Parsing Data(11) section (length: 0x%x)\n", slen);
                ...
                for (uint32_t s = 0; s < seg_count; s++) {
                    ...
                    ASSERT(offset + size <= m->memory.pages * pow(2, 16),
                           ...
                           (uint32_t) (m->memory.pages * pow(2, 16)));trace(
                ...
                break;
            case 10:trace("Parsing Code(10) section (length: 0x%x)\n", slen);
